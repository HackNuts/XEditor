...
Simditor = (function(superClass) {
  ...
  Simditor.prototype._render = function() {
    ...
    this.el = $(this._tpl).insertAfter(this.textarea);
    this.textarea.hide();
...
CodePopover = (function(superClass) {
  ...
  CodePopover.prototype.render = function() {
    ...
    this.selectEl.on('change', (function(_this) {
      return function(e) {
        ...
        if (_this.lang !== -1) {
          return _this.target.find('code').removeClass().addClass('lang-' + _this.lang);
        }
      };
    })(this));
    return this.editor.on('valuechanged', (function(_this) {
  ...
  CodePopover.prototype.show = function() {
    var args, codeClassName;
    args = 1 <= arguments.length ? slice.call(arguments, 0) : [];
    CodePopover.__super__.show.apply(this, args);
    codeClassName = this.target.find('code').attr('class');
    if (codeClassName) {
    ...
    if (this.lang) {
      ...
      this.target.find('code').removeAttr('class');
...
MarkdownButton = (function(superClass) {
  extend(MarkdownButton, superClass);

  function MarkdownButton() {
    return MarkdownButton.__super__.constructor.apply(this, arguments);
  }
...
function createIcon(name, options) {
  ...
  if (shortcut) {
    ...
    link.title += shortcut;
    link.title = link.title.replace('Cmd', '⌘');
    if (isMac) {
      link.title = link.title.replace('Alt', '⌥');
    }
  }

  link.className = "meditor-icon-" + name;
  link.innerHTML = "<span class='fa fa-" + options.icon + "'></span>";
  el.appendChild(link);

  if (name === 'image') {
    var imageMenu = document.createElement('div');
    imageMenu.className = "toolbar-menu toolbar-menu-image";
    imageMenu.innerHTML = "<ul>"
      + "<li><a class='menu-item menu-item-upload-image' href='javascript:;' title='上传图片'><span>上传图片</span></a></li>"
      + "<li><a class='menu-item menu-item-external-image' href='javascript:;' title='外链图片'><span>外链图片</span></a></li>"
      + "</ul>"
      + "<input type='file' class='input-upload-image' title='上传图片' accept='image/*'>";

    el.appendChild(imageMenu);
  }
...
function drawImage(editor, url) {
  ...
  if ((typeof url !== 'undefined') && url !== "") {
    _replaceSelection(cm, stat.image, '![', '](' + url + ')');
  } else {
...
function cancelMarkdown(editor) {
  editor.parent.switchToRich();
}
    ...
    (function (item) {
      ...
      if (item.name === 'image') {
        ...
        $(document).click(function () {
          $(el).removeClass('menu-on');
        });

        // Init uploader
        self.uploader = simple.uploader(self.options.upload);

        // Upload success
        self.uploader.on('uploadsuccess', function (e, file, result) {
          drawImage(self, result.file_path);
        });

        // Open file select dialog
        $(el).find('.menu-item-upload-image').on('click', function (e) {
          $(el).find('.input-upload-image').click();
        });

        // Upload file
        $(el).find('.input-upload-image').on('change', function () {
          self.uploader.upload(this.files);
        });

        // Insert external image
        $(el).find('.menu-item-external-image').click(function () {
          drawImage(self);
        });
      }
...
Editor.prototype.cancelMarkdown = function () {
  cancelMarkdown(this);
};
Editor.prototype.toggleUnderline = function () {
